"""Migration round-trip tests for the provider_connections table (c49243d65a23).
...
def test_migration_downgrade_and_upgrade_round_trip() -> None:
    """Downgrading past c49243d65a23 then upgrading back to it succeeds
    cleanly, and the provider_connections table's presence tracks each
    step.
    """
    ...
    try:
        _run_alembic("downgrade", "0d7ba5dd4102", database_url=test_url)
        with engine.connect() as connection:
            assert "provider_connections" not in inspect(connection).get_table_names()

        _run_alembic("upgrade", "c49243d65a23", database_url=test_url)
        with engine.connect() as connection:
            assert "provider_connections" in inspect(connection).get_table_names()
    finally:
...
def test_orm_metadata_matches_database_after_round_trip() -> None:
    ...
    try:
        ...
        before = set(_VERSIONS_DIR.glob("*_provider_connection_drift_check_scratch.py"))
        _run_alembic(
            "revision",
            "--autogenerate",
            "-m",
            "provider_connection_drift_check_scratch",
            database_url=test_url,
        )
        after = set(_VERSIONS_DIR.glob("*_provider_connection_drift_check_scratch.py"))
